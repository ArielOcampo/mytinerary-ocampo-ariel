<div className="itineraries-background flex flex-wrap items-center justify-center w-full ">
  ...
  {itineraries.map(item =>
    <div className="bg-white dark:bg-gray-800 shadow rounded my-10">
      <div className="relative ">
        <img className="image-itineraries h-96 shadow rounded-t w-full object-cover object-center" src="https://s.inspirockcdn.com/ds10/photos/United%20Arab%20Emirates/3/burj-khalifa-520352452.jpg" alt="Photograpy city" />
        <div className="inset-0 m-auto w-24 h-24 absolute bottom-0 -mb-12 xl:ml-10 rounded border-2 shadow border-white">
          <img className="w-full h-full overflow-hidden object-cover rounded" src={item.creator.image} alt="Photograpy city" />
        </div>
      </div>
      ...
      <div className="px-5 xl:px-10 pb-10 ">
        ...
        <div className="flex justify-center xl:justify-center w-full pt-16 xl:pt-5">
          ...
          <div className="flex items-center">
            <div className="text-4xl text-gray px-10 py-1 font-normal rounded-full">{itineraries.length > 0 && item.itinerary}</div>
          </div>
        </div>
        <div className="pt-3 xl:pt-5 flex flex-col xl:flex-row items-start xl:items-center justify-between">
          <div className="xl:pr-16 w-full xl:w-2/3">
            <div className="text-center xl:text-left mb-3 xl:mb-0 flex flex-col xl:flex-row items-center justify-between xl:justify-start">
              <h2 className="mb-3 xl:mb-0 xl:mr-4 text-2xl text-gray-800 dark:text-gray-100 font-medium tracking-normal">{item?.creator.name}</h2>

            </div>
            <p className="text-center xl:text-left mt-2 text-lg tracking-normal text-gray-600 dark:text-gray-400 leading-5">{item?.hashtags}</p>
          </div>

          <div className="xl:px-10 xl:border-l xl:border-r w-full py-5 flex items-start justify-center ">

            <div className="mr-6 xl:mr-10 ">
              <h2 className="flex flex-grow text-gray-600 dark:text-gray-400 font-bold text-md   mb-2 text-center">{item.price}</h2>
              <p className="text-gray-800 dark:text-gray-100 text-sm xl:text-xl leading-5">Price</p>
            </div>
            <div className="mr-6 xl:mr-10">
              <h2 className="text-gray-600 dark:text-gray-400 font-bold text-xl xl:text-1xl leading-6 mb-2 text-center">{item.duration}HS</h2>
              <p className="flex text-gray-800 dark:text-gray-100 text-sm xl:text-xl leading-5">Duration <img src={Watch} alt="" /></p>
            </div>
            <div>
              <h2 className="text-gray-600 dark:text-gray-400 font-bold text-xl xl:text-1xl leading-6 mb-2 text-center">5</h2>
